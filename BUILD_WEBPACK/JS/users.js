...
goToFriends = function goToFriends(button) {
  window.location = "/user/" + button.id.split('_')[1] + "/friends";
};

goToNews = function goToNews(button) {
  window.location = "/user/" + button.id.split('_')[1] + "/news";
};

openEditWindow = function openEditWindow(button) {
  ...
  $('.modal-body').attr("id", "modal_" + id); //$('.modal-body').id="modal_"+ id;
};

submitEdit = function submitEdit() {
  var actionUrl = "/user/" + $('.modal-body')[0].id.split("_")[1] + "/edit";
  $("#modalForm").attr('action', actionUrl);
  console.log("SUBMIT EDIT WORKDS");
  $("#myModal").modal("hide");
};

showPhoto = function showPhoto(link) {
  var url = link.id;
...
renderUsers = function renderUsers() {
  ...
  $.get("/users", {
    ...
    for (var v in users) {
      $("#users_table").append('<tr><td>' + users[v].id + '</td><td>' + users[v].full_name + '</td><td>' + users[v].birth_date + '</td><td>' + users[v].email + '</td><td><a onclick = showPhoto(this) id=/images/' + users[v].photo + '> <span class=user_photo>Photo </span></a></td><td>' + users[v].role + '</td><td>' + users[v].status + '</td>' + "<td><i class=\"fa-solid fa-user-group\" onclick=goToFriends(this) id=friends_" + users[v].id + "> </i><i class=\"fa-solid fa-newspaper\" onclick=goToNews(this) id=news_" + users[v].id + "></i>" + "<i class=\"fa-solid fa-pen\" id=" + "edit_" + users[v].id + " onclick=openEditWindow(this)> </i></td>" + '</tr>');
    }
  });
};
...
(() => {
/*!**********************************************!*\
  !*** ./webpack_entry_point/webpack_users.js ***!
  \**********************************************/
__webpack_require__(/*! ../../../../CSS/index.less */ "./CSS/index.less");

__webpack_require__(/*! ../../../../JS_WEBPACK/users.js */ "./JS_WEBPACK/users.js");
})();
